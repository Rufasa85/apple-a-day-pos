          ...
          <div className="px-8 py-6 grid grid-cols-3 items-center relative mx-auto">
                    ...
                    {navigation.map((item) => (
                      <Link
                        ...
                        className={classCondition(
                          item.name.toLowerCase() === pathname.split('/')[1]
                            ? 'bg-gray-900 text-white cursor-default peer/active'
                            : 'hover:bg-gray-200 ring-inset ring-2 ring-gray-50',
                          'w-36 rounded-md px-4 py-2 font-semibold text-sm flex justify-center items-center'
                        )}
            ...
            <div className="flex justify-end sm:hidden">
              ...
              <Disclosure.Button className="inline-flex items-center justify-center rounded-md p-1 hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white">
                ...
                {open ? (
                  <XMarkIcon className="stroke-slate-300 block h-8 w-8" aria-hidden="true" />
                ) : (
                  <Bars3Icon className="stroke-slate-300 block h-8 w-8" aria-hidden="true" />
                )}
              </Disclosure.Button>
            </div>
          </div>
            ...
            <div className="gap-2 grid grid-flow-row px-2 pb-3 pt-2 bg-gray-50 text-right">
              {navigation.map((item) => (
                <Link
                  ...
                  to={item.href}
                  className={classCondition(
                    item.name.toLowerCase() === pathname.split('/')[1] ? 'bg-gray-800 text-white' : 'hover:bg-gray-200',
                    'block rounded-md px-8 py-2 text-base font-medium'
                  )}
                  aria-current={item.current ? 'page' : undefined}
                >
                  {item.name}

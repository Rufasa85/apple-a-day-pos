					...
					<Combobox.Options className='absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm'>
						{filteredPeople.length === 0 && customer !== {} ? (
							...
							filteredPeople.map((customerObj) => (
								<Combobox.Option key={customerObj.id} className={({ active }) => `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? 'bg-teal-600 text-white' : 'text-gray-900'}`} value={customerObj}>
									{({ customer, active }) => (
										<>
											<span className={`block truncate ${customer ? 'font-medium' : 'font-normal'}`}>{customerObj.name}</span>
											{customer ? (
